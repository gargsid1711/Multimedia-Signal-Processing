...
def forwardprop(X, W1, W2):
    ...
    H = np.zeros((X_shape[0], X_shape[1], W1_shape[3]))
    E1 = np.zeros((X_shape[0], X_shape[1], W1_shape[3]))
    E2 = np.zeros((X_shape[0], X_shape[1], W2_shape[3], W2_shape[4]))
    Yhat = np.zeros((X_shape[0], X_shape[1], W2_shape[3], W2_shape[4]))

    E1 = conv_layer(X, W1, 1)
    H = np.maximum(0, E1)

    for n1 in range(E2.shape[0]):
        for n2 in range(E2.shape[1]):
            for a in range(E2.shape[2]):
                for k in range(E2.shape[3]):
                    for d in range(W2.shape[2]):
                        E2[n1, n2, a, k] += W2[0, 0, d, a, k] * H[n1, n2, d]
...
def detect(Yhat, number_to_return, anchors):
    ...
    '''
                # (np.array([(rect[0]-anchor[0])/anchor[2],(rect[1]-anchor[1])/anchor[3],
                #  np.log(rect[2]/anchor[2]),np.log(rect[3]/anchor[3])]))
    ...
    # print(Yhat[:, :, :, 4].shape)
    # # print(Yhat[:, :, :, 4])
    # idx = (-Yhat[:, :, :, 4]).argsort(axis=None)#[:number_to_return]
    # # print(idx)
    # unravel = np.unravel_index(idx, (N1, N2, NA))
    # print(len(unravel))

    idx = np.empty(number_to_return, dtype=np.int)
    for i in range(number_to_return):
        ith = np.argpartition(Yhat[:, :, :, 4], (N1*N2*NA-1-i), axis=None)
        ith = ith[N1*N2*NA-1-i]
        idx[i] = ith
    print(idx)

    best_rects = (Yhat[:, :, :, 0:4].reshape((N1*N2*NA, 4)))[idx, :]
    anchors = anchors.reshape((N1*N2*NA, 4))
    anchors = anchors[idx, :]
    best_rects[:,0:2] = anchors[:, 0:2]+best_rects[:, 0:2]*anchors[:, 2:4]
    best_rects[:,2:4] = np.exp(best_rects[:, 2:4])
    best_rects[:,2:4] = best_rects[:,2:4]*anchors[:,2:4]
...
def loss(Yhat, Y):
    ...
    for n1 in range(N1):
        for n2 in range(N2):
            for a in range(NA):
                loss += Y[n1, n2, a, 4] * np.square(np.linalg.norm(Y[n1, n2, a, 0:4] - Yhat[n1, n2, a, 0:4]))
    mse_loss = loss/(2*np.sum(Y[:, :, :, 4], axis=None))

    bce_loss = (-1/(N1*N2*NA)) * np.sum((Y[:, :, :, 4] * np.log(Yhat[:, :, :, 4]) + ((1-Y[:, :, :, 4]) * np.log((1-Yhat[:, :, :, 4])))), axis=None)
...
def backprop(Y, Yhat, H, W2):
    ...
    for n1 in range(N1):
        for n2 in range(N2):
            for d in range(ND):
                if H[n1, n2, d] > 0:
                    GradXi1[n1, n2, d] = np.sum(GradXi2[n1, n2, :, :] * W2[0, 0, d, :, :], axis=None)
        ...
        for m2 in range(-1, 2):
            for c in range(NC):
                for d in range(ND):
                    for n1 in range(N1):
                        for n2 in range(N2):
                            if (n1-m1) < 0 or (n2-m2) < 0 or (n1-m1) >= N1 or (n2-m2) >= N2:
                                dW1[m1+1, m2+1, c, d] += 0
                            else:
